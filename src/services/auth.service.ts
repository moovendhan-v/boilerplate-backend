...
export class AuthService {
   ...
   */
  private generateTokens(user: SafeUser): { accessToken: string; refreshToken: string; tokenId: string } {
   ...
   */
  private async storeRefreshToken(userId: string, refreshToken: string, tokenId: string): Promise<void> {
   ...
   * @returns Authentication response
   */
  async signup(data: { email: string; password: string; name?: string }, res: Response): Promise<AuthResponse> {
    try {
      ...
      const { password: _, ...safeUser } = user;
      const { accessToken, refreshToken, tokenId } = this.generateTokens(safeUser);
  ...
  async refreshToken(refreshToken: string): Promise<{ access_token: string; refreshToken: string }> {
    try {
      ...
      if (!tokenData) {
        logger.warn(`Token refresh failed: Token not found in Redis for user ${userId}`);
      ...
      const { accessToken, refreshToken: newRefreshToken, tokenId: newTokenId } = this.generateTokens(safeUser);
      ...
      if (error instanceof jwt.JsonWebTokenError) {
        logger.warn('Token refresh failed: JWT validation error', { error: error.message });
        throw new GraphQLError('Invalid refresh token', {
          extensions: { code: 'UNAUTHORIZED' },
        });
      }
